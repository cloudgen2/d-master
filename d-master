...
class Temp(AppHistory):
    # Class Required: CmdHistory, MsgBase, Which, Sh, Shell, AppData, AppHistory
...
class DockerConfig(object):
        ...
        if self.configFile() != '':
            ...
            for l in self.foundList:
                ...
                if dockerPath !="":
                    ...
                    self.prn("Container cleaned: %s" % self.dockerName())
    ...
    def maintainer(self, maintainer=None):
        if maintainer is not None:
            self.__maintainer__=re.sub(r"\s+", "", maintainer).lower()
            return self
...
class DockerBase(AppBase, DockerConfig, DockerBuild, ContainerBase, ImageBase):
                ...
                if self.cmd() == 'build':
                    ...
                    if result:
                        if self.testProfile():
                            ...
                            tag = self.docker_tag()
                        self.safeMsg('Using profile: "%s". Architecture: "%s". DockerName: "%s".' % (self.profile(),arch, dockerName),'PROFILE')
                    if dockerName !="":
...
if __name__ == "__main__":
    ...
    app.setInstallation(appName='d-master',author='Cloudgen Wong',homepage="https://github.com/cloudgen2/d-master",downloadUrl="https://dl.leolio.page/d-master",lastUpdate='2024-5-7',majorVersion=17,minorVersion=7)
    app.start()
